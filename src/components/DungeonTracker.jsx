...
const DUNGEONS = [
  {
    ...
    scryfallName: 'Dungeon of the Mad Mage', scryfallSet: 'tafr',
  ...
  },
  {
    ...
    scryfallName: 'Lost Mine of Phandelver', scryfallSet: 'tafr',
  ...
  },
  {
    id: 'undercity',
    name: 'The Undercity',
    scryfallName: 'The Undercity', scryfallSet: 'tclb',
...
function useDungeonImage(name, scryfallSet) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    let cancelled = false
    fetch('https://api.scryfall.com/cards/named?fuzzy=' + encodeURIComponent(name) + '&set=' + (scryfallSet || 'tafr'))
...
function PlayerDungeon({ player }) {
  ...
  const { url: cardImg, loading: imgLoading } = useDungeonImage(dungeon.scryfallName, dungeon.scryfallSet)
