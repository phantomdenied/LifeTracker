import { useState, useEffect } from 'react'
import './DungeonTracker.css'
// Each room: { id, name, effect, next: [id, ...] }
// next: [] = terminal (dungeon complete)
// next: [single] = linear
// next: [a, b] = branch choice
const DUNGEONS = [
{
id: 'mad-mage',
name: 'Dungeon of the Mad Mage',
scryfallName: 'Dungeon of the Mad Mage',
startRoom: 'yawning-portal',
rooms: {
'yawning-portal': { name: 'Yawning Portal', effect: 'You gain 1 life',
'dungeon-level': { name: 'Dungeon Level', effect: 'Scry 1',
'goblin-bazaar': { name: 'Goblin Bazaar', effect: 'Create a Treasure token',
'twisted-caverns': { name: 'Twisted Caverns', effect: "Target creature can't attack
'lost-level': { name: 'Lost Level', effect: 'Scry 2',
'runestone-caverns': { name: 'Runestone Caverns', effect: 'Exile the top two cards of y
'muirals-graveyard': { name: "Muiral's Graveyard",effect: 'Create two 1/1 black Skeleto
'deep-mines': { name: 'Deep Mines', effect: 'Scry 3',
'mad-wizards-lair': { name: "Mad Wizard's Lair", effect: 'Draw three cards and reveal
},
},
{
id: 'phandelver',
name: 'Lost Mine of Phandelver',
scryfallName: 'Lost Mine of Phandelver',
startRoom: 'cave-entrance',
rooms: {
'cave-entrance': { name: 'Cave Entrance', effect: 'Scry 1',
'goblin-lair': { name: 'Goblin Lair', effect: 'Create a 1/1 red 'mine-tunnels': { name: 'Mine Tunnels', effect: 'Create a Treasure token
'storeroom': { name: 'Storeroom', effect: 'Put a +1/+1 counter on
'dark-pool': { name: 'Dark Pool', effect: 'Each opponent loses 1 l
'fungi-cavern': { name: 'Fungi Cavern', effect: 'Target creature gets −4
Goblin
'temple-of-dumathoin': { name: 'Temple of Dumathoin', effect: 'Draw a card',
},
},
{
id: 'undercity',
name: 'The Undercity',
scryfallName: 'The Undercity',
life',
startRoom: 'dungeon-entrance',
rooms: {
'dungeon-entrance': { name: 'Dungeon Entrance', effect: 'You gain 1 'forge': { name: 'Forge', effect: 'Create a Treasure
'lost-well': { name: 'Lost Well', effect: 'Scry 2',
'trap': { name: 'Trap!', effect: 'Each opponent los
'arena': { name: 'Arena', effect: 'Target creature f
'sewer': { name: 'Sewer', effect: 'Surveil 2',
'throne': { name: 'Throne of the Dead Three', effect: 'Each opponent sac
},
},
]
const imgCache = {}
function useDungeonImage(name) {
const [url, setUrl] = useState(imgCache[name] ?? null)
const [loading, setLoading] = useState(!imgCache[name])
useEffect(() => {
if (!name) return
if (imgCache[name]) { setUrl(imgCache[name]); setLoading(false); return }
setLoading(true)
let cancelled = false
fetch(`https://api.scryfall.com/cards/named?fuzzy=${encodeURIComponent(name)}`)
.then(r => { if (r.ok) return r.json(); throw new Error() })
.then(card => {
const img =
card.image_uris?.large ??
card.image_uris?.normal ??
card.card_faces?.[0]?.image_uris?.large ??
card.card_faces?.[0]?.image_uris?.normal ??
null
if (!cancelled) { if (img) imgCache[name] = img; setUrl(img) }
})
.catch(() => { if (!cancelled) setUrl(null) })
.finally(() => { if (!cancelled) setLoading(false) })
return () => { cancelled = true }
}, [name])
return { url, loading }
}
function PlayerDungeon({ player }) {
const [dungeonId, setDungeonId] = useState(DUNGEONS[0].id)
// currentRoom: null = not started, string = room id
const [currentRoom, setCurrentRoom] = useState(null)
// visitedRooms: Set of room ids visited in order
const [visited, setVisited] = useState([])
const dungeon = DUNGEONS.find(d => d.id === dungeonId)
const room = currentRoom ? dungeon.rooms[currentRoom] : null
const nextRooms = room ? room.next : []
const completed = currentRoom && nextRooms.length === 0
const { url: cardImg, loading: imgLoading } = useDungeonImage(dungeon.scryfallName)
function changeDungeon(id) {
setDungeonId(id)
setCurrentRoom(null)
setVisited([])
}
function venture(roomId) {
setCurrentRoom(roomId)
setVisited(v => [...v, roomId])
}
function reset() {
setCurrentRoom(null)
setVisited([])
}
// Build an ordered flat list of rooms for display
// We show visited rooms (in order), then the available next choices
const roomEntries = Object.entries(dungeon.rooms)
return (
<div className="player-dungeon" style={{ '--player-color': player.color }}>
<div className="pd-header">
<span className="pd-name" style={{ color: player.color }}>{player.name}</span>
<select className="dungeon-select" value={dungeonId} onChange={e => changeDungeon(e.t
{DUNGEONS.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
</select>
</div>
<div className="pd-body">
<div className="dungeon-rooms">
{!currentRoom && (
<div className="dungeon-not-started">
Not yet ventured — tap Venture ▶ to begin
</div>
)}
{/* Visited rooms */}
{visited.map((rid, i) => {
const r = dungeon.rooms[rid]
const isCurrent = rid === currentRoom
return (
<div
key={rid + i}
className={['room-btn', isCurrent ? 'current' : 'done'].filter(Boolean).join(
>
<div className="room-top">
<span className="room-marker">{isCurrent ? '▶' : '✓'}</span>
<span className="room-name">{r.name}</span>
</div>
<div className="room-effect">{r.effect}</div>
</div>
)
})}
{/* Branch choices — shown when current room has multiple nexts */}
{!completed && nextRooms.length > 1 && (
<div className="branch-prompt">
<span className="branch-label">Choose your path:</span>
<div className="branch-choices">
{nextRooms.map(rid => {
const r = dungeon.rooms[rid]
return (
<button key={rid} className="branch-btn" onClick={() => venture(rid)}>
<span className="room-name">{r.name}</span>
<span className="room-effect">{r.effect}</span>
</button>
)
})}
</div>
</div>
)}
</div>
{/* Dungeon card image */}
<div className="dungeon-card-col">
<div className="dungeon-card-wrap">
{imgLoading && <div className="dungeon-img-skeleton" />}
{cardImg && !imgLoading && (
<img className="dungeon-card-img" src={cardImg} alt={dungeon.name} loading="laz
)}
{!imgLoading && !cardImg && (
<div className="dungeon-img-fallback">{dungeon.name}</div>
)}
</div>
</div>
</div>
{completed && <div className="dungeon-complete">✓ Dungeon Complete!</div>}
<div className="dungeon-actions">
{/* Venture only shown when linear (single next room) */}
{!completed && nextRooms.length === 1 && (
<button
className="dungeon-advance"
onClick={() => venture(nextRooms[0])}
>
Venture ▶
</button>
)}
{/* Start button when not yet begun */}
{!currentRoom && (
<button
className="dungeon-advance"
onClick={() => venture(dungeon.startRoom)}
>
Venture ▶
</button>
)}
{completed && (
<button className="dungeon-advance" onClick={reset}>
Venture Again
</button>
)}
</div>
</div>
<button className="dungeon-reset" onClick={reset}>Reset</button>
)
}
export default function DungeonTracker({ players }) {
return (
<div className="dungeon-tracker">
{players.filter(p => !p.eliminated).map(p => (
<PlayerDungeon key={p.id} player={p} />
))}
</div>
)
}
