...
// Cache so we don’t re-fetch same card
const imgCache = {}
...
```
// Search with type:plane to avoid matching regular cards with the same name (e.g. Shivan Gorge)
fetch(`https://api.scryfall.com/cards/search?q=!"${encodeURIComponent(planeName)}"+type:plane&order=released`)
  .then(r => {
  ...
  .then(data => {
    ...
    if (!card) throw new Error('no results')
    const img =
